...
To inspect the queries generated by the MongoDB engine during debugging, enable command logging. 
This can be achieved by deriving from the **DataConfiguration** class and overriding the **LogCommand** method. 
The user can then implement their preferred method for logging the given command.

Code Samples
""""""""""""""""""

.. code-block:: c#
    :caption: Enabling Command Logging for MongoDB Debugging.
    ...
    {
        public override void LogCommand(Command command)
        {
            logger.Log(new LogInfo(LogLevel.Information, "CommmandLog", $"CommandName: {command.Name} CommandText: {command.JsonText}"));
        }
    }
...
In some cases, you may need to modify the default database initialization process. VersaTul.Data.MongoDB allows you to do this quickly and easily.

To achieve this, derive from the **DataConfiguration** class and override the **GetDatabaseFromUrl** method. 
...
* Added mongo command logging 
* Mongo database configuration override. 
* Minor fixes
